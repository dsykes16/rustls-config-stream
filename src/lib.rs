...
///
/// # Contract
/// - `build()` should return a stream that eventually yields at least one
///   `ServerConfig` during initial startup. If it doesn't, startup will fail
///   with [`ServerConfigStreamError::EmptyStream`].
/// - On stream failure, the provider will call `build()` again with backoff.
...
///
/// # Concurrency
/// Reads (`get_config`) are wait-free and do not block updates. Updates occur
/// on a background task that listens to the user-provided stream.
///
/// # Backoff & Recovery
...
/// # use std::sync::Arc;
/// # use rustls::ServerConfig;
/// # use your_crate::{ServerConfigProvider, ServerConfigStreamBuilder};
/// # async fn example(mut builder: impl ServerConfigStreamBuilder + Send + 'static) -> anyhow::Result<()> {
/// let provider: Arc<ServerConfigProvider> = ServerConfigProvider::start(builder).await?;
///
/// // Later, when accepting connections:
/// let cfg: Arc<ServerConfig> = provider.get_config();
/// // Use `cfg` with your TLS acceptor/handshake.
/// # Ok(())
/// # }
/// ```
...
impl ServerConfigProvider {
    ...
    ///
    /// This is a cheap, lock-free read that clones the internal `Arc`.
